// EPI.cpp : Defines the class behaviors for the application.
//

#include "stdafx.h"
#include "EPI.h"

#include "MainFrm.h"
#include "EPIDoc.h"
#include "EPIView.h"
#include "FormImageSelect.h"
#include "EPILogHelper.h"
#include "EPIController.h"
#include "PageEPI.h"

#ifdef _DEBUG
#define new DEBUG_NEW
#undef THIS_FILE
static char THIS_FILE[] = __FILE__;
#endif

/////////////////////////////////////////////////////////////////////////////
// CEPIApp

BEGIN_MESSAGE_MAP(CEPIApp, CWinApp)
    //{{AFX_MSG_MAP(CEPIApp)
    ON_COMMAND(ID_APP_ABOUT, OnAppAbout)
    ON_COMMAND(ID_FILE_OPEN, OnFileOpen)
    ON_COMMAND(ID_FILE_NEW, OnFileNew)
    //}}AFX_MSG_MAP
    // Standard file based document commands
    ON_COMMAND(ID_FILE_NEW, CWinApp::OnFileNew)
    ON_COMMAND(ID_FILE_OPEN, CWinApp::OnFileOpen)
    // Standard print setup command
    ON_COMMAND(ID_FILE_PRINT_SETUP, CWinApp::OnFilePrintSetup)
END_MESSAGE_MAP()

/////////////////////////////////////////////////////////////////////////////
// CEPIApp construction

CEPIApp::CEPIApp()
{
    // TODO: add construction code here,
    // Place all significant initialization in InitInstance
}

/////////////////////////////////////////////////////////////////////////////
// The one and only CEPIApp object

CEPIApp theApp;

/////////////////////////////////////////////////////////////////////////////
// CEPIApp initialization

BOOL CEPIApp::InitInstance()
{
    CEPILogHelper::CreateInstance();
    {
        // BLOCK: doc template registration
        // Register the document template.  Document templates serve
        // as the connection between documents, frame windows and views.
        // Attach this form to another document or frame window by changing
        // the document or frame class in the constructor below.
        CSingleDocTemplate* pNewDocTemplate = new CSingleDocTemplate(
            IDR_EPIFORMVIEW_TMPL,
            RUNTIME_CLASS(CEPIDoc),     // document class
            RUNTIME_CLASS(CMainFrame),      // frame class
            RUNTIME_CLASS(CFormImageSelect));       // view class
        AddDocTemplate(pNewDocTemplate);
    }
    AfxEnableControlContainer();
    // Standard initialization
    // If you are not using these features and wish to reduce the size
    //  of your final executable, you should remove from the following
    //  the specific initialization routines you do not need.
#ifdef _AFXDLL
//    Enable3dControls();         // Call this when using MFC in a shared DLL
#else
    Enable3dControlsStatic();   // Call this when linking to MFC statically
#endif
    // Change the registry key under which our settings are stored.
    // TODO: You should modify this string to be something appropriate
    // such as the name of your company or organization.
    SetRegistryKey(_T("Local AppWizard-Generated Applications"));
    LoadStdProfileSettings();  // Load standard INI file options (including MRU)
    // Register the application's document templates.  Document templates
    //  serve as the connection between documents, frame windows and views.
    //CSingleDocTemplate* pDocTemplate;
    pDocTemplate = new CSingleDocTemplate(
        IDR_MAINFRAME,
        RUNTIME_CLASS(CEPIDoc),
        RUNTIME_CLASS(CMainFrame),       // main SDI frame window
        RUNTIME_CLASS(CEPIView));
    AddDocTemplate(pDocTemplate);
    // Parse command line for standard shell commands, DDE, file open
    CCommandLineInfo cmdInfo;
    ParseCommandLine(cmdInfo);

    // Dispatch commands specified on the command line
    //cmdInfo.m_nShellCommand = CCommandLineInfo::FileNothing;
    if(!ProcessShellCommand(cmdInfo)) {
        return FALSE;
    }

    // The one and only window has been initialized, so show and update it.
    CString cs(_T("Artifact Rejection Tool For EPI"));
    m_pMainWnd->SetWindowText(cs);
    m_pMainWnd->ShowWindow(SW_SHOWMAXIMIZED);
    m_pMainWnd->UpdateWindow();
    return TRUE;
}


/////////////////////////////////////////////////////////////////////////////
// CAboutDlg dialog used for App About

class CAboutDlg : public CDialog
{
public:
    CAboutDlg();

// Dialog Data
    //{{AFX_DATA(CAboutDlg)
    enum { IDD = IDD_ABOUTBOX };
    //}}AFX_DATA

    // ClassWizard generated virtual function overrides
    //{{AFX_VIRTUAL(CAboutDlg)
protected:
    virtual void DoDataExchange(CDataExchange* pDX);    // DDX/DDV support
    //}}AFX_VIRTUAL

// Implementation
protected:
    //{{AFX_MSG(CAboutDlg)
    //}}AFX_MSG
    DECLARE_MESSAGE_MAP()
};

CAboutDlg::CAboutDlg() : CDialog(CAboutDlg::IDD)
{
    //{{AFX_DATA_INIT(CAboutDlg)
    //}}AFX_DATA_INIT
}

void CAboutDlg::DoDataExchange(CDataExchange* pDX)
{
    CDialog::DoDataExchange(pDX);
    //{{AFX_DATA_MAP(CAboutDlg)
    //}}AFX_DATA_MAP
}

BEGIN_MESSAGE_MAP(CAboutDlg, CDialog)
    //{{AFX_MSG_MAP(CAboutDlg)
    //}}AFX_MSG_MAP
END_MESSAGE_MAP()

// App command to run the dialog
void CEPIApp::OnAppAbout()
{
    CAboutDlg aboutDlg;
    aboutDlg.DoModal();
}

/////////////////////////////////////////////////////////////////////////////
// CEPIApp message handlers


void CEPIApp::OnFileOpen()
{
    // TODO: Add your command handler code here
    TCHAR szFilters[] = _T("All Files (*.*)|*.*||");
    CFileDialog FileDialog(TRUE, NULL, NULL,
                           OFN_FILEMUSTEXIST | OFN_HIDEREADONLY, szFilters);
    // Create buffer for ten filenames
    TCHAR strFiles[MAX_PATH * 10] = "";
    FileDialog.m_ofn.lpstrFile = strFiles;
    FileDialog.m_ofn.nMaxFile  = sizeof(strFiles);

    if(FileDialog.DoModal() == IDOK) {
        CString fileName = FileDialog.GetPathName();
        //CMDIFrameWnd *pFrame = (CMDIFrameWnd*)AfxGetApp()->GetMainWnd();
        // Get the active MDI child window.
        //CMainFrame *pChild = (CMainFrame*)pFrame->GetActiveFrame();
        //pChild->setTextForView2();
        //CFormImageSelect *pWnd = pChild->GetEPIFormView();
        //pWnd->setStudyName(fileName);
        (CPageEPI::GetPageOneInstance())->setStudyName(fileName);
        (CEPIController::GetInstance())->setStudyFileName(fileName);
        (CEPIController::GetInstance())->loadStudy(0);
    }
}


void CEPIApp::OnFileNew()
{
    // TODO: Add your command handler code here
    //CWinApp::OnFileNew();
    pDocTemplate->OpenDocumentFile(NULL);
}

int CEPIApp::ExitInstance()
{
    // TODO: Add your specialized code here and/or call the base class
    CEPIController::DeleteInstance();
    CPageEPI::DeletePageOneInstance();
    CEPILogHelper::DeleteInstance();
    return CWinApp::ExitInstance();
}
