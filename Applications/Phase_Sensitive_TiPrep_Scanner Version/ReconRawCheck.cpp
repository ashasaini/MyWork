...
{
    /*TIPREP_TRACE(CReconRawCheck::CReconRawCheck);*/
}

CReconRawCheck& CReconRawCheck::operator = (CReconRawCheck& f_RawCheck_ptr)
{
    /*TIPREP_TRACE(CReconRawCheck::operator);*/
...
{
    TIPREP_TRACE(CReconRawCheck::RawCheck,MRTraceMarker3);
    ...
    try{        
        ...
        if(FALSE == RawCheckCreateProcess(f_engineHost , f_csStudyFileName ,  f_nProtocolNo)){
        ...
        CTiPrepLogHelper::WriteToErrorLog(_T("Exception occurred in RawCheck"), __LINE__, __FILE__);
        RawCheckCleanUp();
        return FALSE;   
    }
...
{
    TIPREP_TRACE(CReconRawCheck::RawCheckCreateProcess,MRTraceMarker3);
    try
    {
        CString szCmdline("");
        BOOL RawCheckFileWriteStatus = TRUE;
        //gprexec host command    
       // szCmdline.Format(_T("\"C:\\gp\\bin\\rawCheck\" -e %s@%s:/1.%d"),f_engineHost, f_csStudyFileName, f_nProtocolNo);
        ...
        CTiPrepLogHelper::WriteToTraceLog(_T("MRI_TIPREP"), MRTraceMarker3,szCmdline , _T("CReconRawCheck::RawCheckCreateProcess"));
        // Create the child process. 
        ...
        else 
        {
            ...
            RawCheckFileWriteStatus = RawCheckReadFromPipe(); 
        ...
        if(RawCheckFileWriteStatus == FALSE){
            return FALSE;
        }
        return TRUE;
    }catch (...){
        CTiPrepLogHelper::WriteToErrorLog(_T("Exception Occurred in RawCheckCreateProcess"), __LINE__,__FILE__);
        return FALSE;
    }    
...
{ 
    TIPREP_TRACE(CReconRawCheck::RawCheckWriteToPipe,MRTraceMarker3);
...
// Stop when there is no more data. 
{ 
    TIPREP_TRACE(CReconRawCheck::RawCheckReadFromPipe,MRTraceMarker3);
    try {
        DWORD dwRead, dwWritten; 
        CHAR chBuf[BUFSIZE]; 
        BOOL bSuccess = FALSE;
        if(m_hInputFile){
            ...
            {
                CTiPrepLogHelper::WriteToErrorLog(_T("handle value invalid RawCheckReadFromPipe()"),__LINE__,__FILE__);
                return FALSE;
            }
            ...
            if (peekAvail == 0 || bPeek == FALSE) { // Using peekAvail
                CTiPrepLogHelper::WriteToErrorLog(_T("PeekNamedPipe return fail, unable to read from pipe"),__LINE__,__FILE__);
                return FALSE;
            }
            bSuccess = ReadFile( m_hChildStd_OUT_Rd, chBuf, BUFSIZE, &dwRead, NULL);
            CString trace = _T("ReadFile() executed");
            
            CTiPrepLogHelper::WriteToTraceLog(_T("MRI_TIPREP"),MRTraceMarker3,trace,_T("CReconRawCheck::RawCheckReadFromPipe"));
            if( ! bSuccess || dwRead == 0 ){
                CTiPrepLogHelper::WriteToErrorLog(_T("ReadFile() failed in RawCheckReadFromPipe(). Unable to get ouput of raw check"),__LINE__,__FILE__);
                return FALSE; 
            }            
            ...
            if (! bSuccess ) {
                CTiPrepLogHelper::WriteToErrorLog(_T("WriteFile() failed in RawCheckReadFromPipe(). Unable to get ouput of raw check"),__LINE__,__FILE__);
                return FALSE; 
            }
            return TRUE;
        }else{
        ...
        CTiPrepLogHelper::WriteToErrorLog(_T("Exception occurred in RawCheckReadFromPipe()"),__LINE__,__FILE__);
        return FALSE;
    }
} 
...
void CReconRawCheck::RawCheckCleanUp()
{
    ...
    {
        CTiPrepLogHelper::WriteToErrorLog(_T("Exception occurred in RawCheckCleanUp()"),__LINE__,__FILE__);
